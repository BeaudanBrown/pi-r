...
Built-in `edit` and `write` are available under:

- non-generated R scripts under `R/`;
- `tests/`; and
- `docs/`.

Contract-declared Approved Function files use direct `r_function_edit`, which preserves their locked signatures without requiring a prior inspection.
...
For broader work, edit function bodies, supporting R scripts, tests, and `docs/analysis-plan.md` together, then run the narrowest useful targets and inspect bounded outputs. Keep consequential assumptions and unresolved questions visible; do not present them as approved conclusions.
