...
`/r revise` verifies provenance and a clean tree, rejects pending environment/approval work, confirms Transient State loss, and seeds the ignored Contract Draft from the committed contract while leaving source unchanged. `/r lock` previews and transactionally commits the revision, preserving bodies of unchanged Approved Function signatures. `/r cancel-revision` discards the draft and restores the unchanged Implementation Mode runtime.
...
The human-facing Operator Status Widget reports selected mode, provenance, contract, approval, and worker facts. It is not called the HUD.
