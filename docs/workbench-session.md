...
`/r revise` verifies provenance and a clean tree, rejects pending environment/approval work, confirms Transient State loss, and seeds the ignored structural Contract Draft while leaving source unchanged. `r_contract_draft_inspect` provides an authoritative compact project overview and bounded function detail. `/r lock` validates and commits topology changes while preserving existing bodies whenever names and signatures are unchanged. Descriptive purpose, requirements, living analysis notes, and tests may change directly in Implementation Mode and never require a behavior-only revision. `/r cancel-revision` discards the draft and restores the unchanged Implementation Mode runtime.
