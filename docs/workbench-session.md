...
`/r revise` verifies provenance and a clean tree, rejects pending environment/approval work, confirms Transient State loss, and seeds the ignored Contract Draft from the committed contract while leaving source unchanged. `r_contract_draft_inspect` returns exactly one compact function record per page, with authoritative returned/next offsets, stable page decision IDs, and a canonical semantic topology-change flag instead of requiring conversational reconstruction. For behavior-only migration, `r_function_behavior_propose` updates purpose, requirements, complete category review, and durable evidence for existing functions while preserving topology; it reports unresolved functions. The agent asks focused questions rather than inferring answers. `/r lock` rejects any unresolved behavior, previews the complete requirements, and transactionally commits the revision, preserving bodies only when signature and behavior are unchanged. `/r cancel-revision` discards the draft and restores the unchanged Implementation Mode runtime.
