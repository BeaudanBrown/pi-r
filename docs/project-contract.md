...
- Every Approved Function lists required parameter names plus a user-approved `purpose` and one or more behavioral `requirements`; bodies, defaults, and variadic arguments are not representable. Requirements record relevant missing-value, duplicate, coding, cohort, and output rules instead of leaving implementation to infer them from names or observed values. Legacy locked contracts without these fields remain readable and existing targets can still run, but affected functions are excluded from effective edit scopes. A user-approved Contract Revision must supply every missing behavior before the draft can be relocked.
