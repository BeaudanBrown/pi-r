...
2. In Design Mode, inspect text through the exposed read tools and inspect uncontracted CSV/TSV inputs through `r_data_inspect`, then propose one complete Project Contract with `r_contract_propose`. Supply only project decisions: pi-r injects contract/template/policy versions and the exact packaged Nixpkgs pin. Represent file outputs through the explicit `output` binding documented in the operational reference; zero functions and targets are valid when the requested design is empty. Wait for user-only `/r lock`.
