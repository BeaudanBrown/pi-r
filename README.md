...
Implementation Mode is an iterative workbench, not a behavior-approval gate. Structural topology remains locked, while the agent may make provisional edits under `R/`, `tests/`, and `docs/`, use the optional Tree-sitter Approved Function editor, and develop analytical behavior through living documentation, synthetic tests, sandboxed execution, and focused user review. Purpose and requirements remain readable descriptive notes but never block implementation. Structural changes use user-only `/r revise`, followed by `/r lock` or `/r cancel-revision`; dependency-only changes remain in the governed environment workflow. See [Provisional implementation](docs/scoped-implementation.md).

All constrained modes expose paginated raw CSV/TSV profiling and ordinary transactional R execution through a lazy, persistent R worker. The compact `r_exec`, `r_inspect`, `r_status`, `r_clear`, and `r_reset` interface hides sandbox and rollback complexity without restricting normal R expressions. Bubblewrap keeps project and attached source read-only; failed evaluations roll back, successful calls retain only explicitly named objects, and retained objects have bounded inspection and temporary-only cleanup tools. Contract lock restarts exploration in the generated project environment. One XML-delimited, non-persistent agent-facing Current-State HUD gives every model call current mode, authority, transition, and bounded worker/object inventory without accumulating stale snapshots or appearing as a user request. See [Persistent sandboxed R exploration](docs/r-worker.md).
